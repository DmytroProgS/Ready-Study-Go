...
import horenTeil4Variants from '../../../data/horenTeil4Data';
import '../../../assets/styles/lesen.css';

function HorenTeil4Page() {
  const [variantIndex, setVariantIndex] = useState(0);
  const [answers, setAnswers] = useState({});
  const [submitted, setSubmitted] = useState(false);
  const variant = horenTeil4Variants[variantIndex];
      ...
      section.questions.forEach((q, qi) => {
        ...
        if (q.alts) {
          q.alts.forEach((alt, ai) => {
            list.push({
              number: q.number,
              type: alt.options ? 'abc' : 'richtig-falsch',
  ...
  const handleNext = () => {
    const nextIndex = (variantIndex + 1) % horenTeil4Variants.length;
  ...
  return (
    <div className="lesen-page">
      <div className="lesen-header">
        ...
        <h1>H&ouml;ren Teil 4</h1>
        <div className="lesen-variant-nav">
          {horenTeil4Variants.map((v, i) => (
            <button
              key={v.id}
      ...
      <p className="lesen-subtitle">
        Variante {variant.label} / {horenTeil4Variants.length} &mdash; H&ouml;ren Sie die Nachrichten und beantworten Sie die Fragen.
      </p>
      ...
      {variant.audioUrl && (
        ...
        >
          &#x1F50A; Audio abspielen (alle Nachrichten)
        </a>
      )}
